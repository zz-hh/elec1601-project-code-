...
int checkRobotSensorFFWalls(struct Robot * robot, struct Wall_collection * head) {
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX+(-ROBOT_WIDTH/-10)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*sin((robot->angle+45)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/-10)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*cos((robot->angle+45)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
void robotUpdate(struct SDL_Renderer * renderer, struct Robot * robot){
    ...
    */
    ...
    for (i = 0; i < 5; i++)
    {
        xDir = round(robotCentreX+(-ROBOT_WIDTH/-10)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*sin((robot->angle+45)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/-10)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*cos((robot->angle+45)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
...
void robotAutoMotorMove(struct Robot * robot, int front_left_sensor, int front_right_sensor, int right_sensor , int ff_sensor) {
   ...
   if(ff_sensor>=1 && right_sensor>=1)
   {
       if (ff_sensor > (right_sensor/0.7071067812))
       {
           robot->angle += 1;
        }
        else
    ...
    if (wallfound==1 && ff_sensor == (right_sensor/0.7071067812))
    {
        robot->currentSpeed = MAX_ROBOT_SPEED;
    }
    if (wallfound==1 && ff_sensor==0)
    {
        robot->currentSpeed = 0;
        robot->angle += 10;
    }
